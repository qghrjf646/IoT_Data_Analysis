...
\begin{itemize}
    \item \textbf{Exploratory Attacks (Evasion):} The attacker manipulates \textit{test-time inputs} to evade a deployed model without modifying the model itself. The model remains unchanged; only the input is perturbed.
    \item \textbf{Causative Attacks (Poisoning):} The attacker manipulates \textit{training data} to corrupt the learned model. The model itself is compromised, affecting all future predictions.
\end{itemize}

\subsection{Exploratory Attack: Fast Gradient Sign Method (FGSM)}
...
\subsection{Causative Attack: Data Poisoning}

Unlike exploratory attacks that manipulate inputs at test time, causative attacks target the \textit{training phase} itself. An attacker with access to training data can inject malicious samples or flip labels to corrupt the learned decision boundary. We demonstrate this using a label flipping attack on a Linear SVM classifier trained on 2D PCA-projected data.

\textbf{Attack Mechanism:}
\begin{enumerate}
    \item Select a fraction of attack samples from the training set
    \item Flip their labels from ``attack'' to ``benign''
    \item Shift the poisoned samples toward the benign centroid
    \item Retrain the model on the corrupted dataset
\end{enumerate}

\begin{table}[H]
\centering
\caption{Causative Attack Results on Linear SVM (2D PCA projection)}
\label{tab:causative_results}
\begin{tabular}{ccc}
\toprule
\textbf{Poison Rate} & \textbf{Test Accuracy} & \textbf{Accuracy Drop} \\
\midrule
0\% (Baseline) & 68.84\% & --- \\
5\% & 68.20\% & -0.64\% \\
10\% & 67.04\% & -1.80\% \\
...
Figure~\ref{fig:causative_boundaries} illustrates how increasing poison rates progressively shift the decision boundary, allowing more attack samples to be misclassified as benign. At 25\% poisoning, accuracy drops by over 8 percentage points, demonstrating the real threat of training data manipulation.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/causative_attack_boundaries.png}
\caption{Decision boundary shift due to data poisoning at various poison rates. The Linear SVM model was trained on 2D PCA-projected data. As poison rate increases, the boundary shifts to misclassify more attack samples as benign.}
\label{fig:causative_boundaries}
\end{figure}

\subsection{Attack Comparison Summary}

\begin{table}[H]
\centering
\caption{Comparison of Adversarial Attack Types}
\label{tab:attack_comparison}
\begin{tabular}{p{3cm}p{5.5cm}p{5.5cm}}
\toprule
\textbf{Aspect} & \textbf{Exploratory (FGSM)} & \textbf{Causative (Poisoning)} \\
\midrule
