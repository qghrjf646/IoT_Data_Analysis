...
\midrule
Isolation Forest & 0.832 & 0.889 & 0.688 \\
One-Class SVM & 0.693 & 0.862 & 0.276 \\
\textbf{LOF} & \textbf{0.844} & \textbf{0.895} & \textbf{0.705} \\
\bottomrule
\end{tabular}
\end{table}
\vspace{0.3cm}
...
\textbf{Fast Gradient Sign Method} -- Perturbs test samples
\vspace{0.2cm}
...
\textbf{Label Flipping} -- Poisons training data to shift decision boundary
\vspace{0.3cm}
...
\begin{itemize}
    ...
    \item Attacks evade detection at inference
\end{itemize}
\vspace{0.3cm}
\begin{table}
\centering
...
\midrule
Linear SVM & 90.2\% & 3.2\% & 3.6\% \\
Random Forest & 94.6\% & 3.8\% & 4.0\% \\
\textbf{Gradient Boosting} & \textbf{94.4\%} & \textbf{34.0\%} & \textbf{36.0\%} \\
\bottomrule
\end{tabular}
\end{table}
\vspace{0.5cm}
\begin{center}
\textbf{Finding:} Gradient Boosting retains 36\% accuracy under FGSM attack\\
...
Adversarial Robustness & Gradient Boosting & 36.0\% robust acc. \\
\bottomrule
\end{tabular}
\end{table}
\vspace{0.5cm}
...
\begin{enumerate}
    ...
    \item \textbf{Layer 2 (Random Forest + Gradient Boosting):} Classification ensemble for accuracy and robustness
